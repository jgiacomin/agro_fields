...
void main() {
  test(
    'ActivoAgroV2 conserva DerechoActivo en toMap/fromMap',
    () {
      ...
      final activo = ActivoAgroV2(
        ...
        descripcion: 'Activo utilizado para prueba de derechos',
      ...
      expect(reconstruido.derechos.length, 1);

      final derechoReconstruido = reconstruido.derechos.first;

      expect(derechoReconstruido.derechoId, 'derecho_001');
      expect(derechoReconstruido.activoAgroId, 'activo_001');
      expect(derechoReconstruido.tipoDerecho, 'dominio');
      expect(
        derechoReconstruido.descripcionTipo,
        'Dominio sobre inmueble rural',
      );
      expect(derechoReconstruido.tipoObjeto, 'inmueble');
      expect(
        derechoReconstruido.descripcionObjeto,
        'Campo rural identificado como activo_001',
      );

      expect(
        derechoReconstruido.alcanceTerritorial?.tipoAlcance,
        'total',
      );
      expect(
        derechoReconstruido.alcanceTerritorial?.superficieAfectada,
        1000,
      );
      expect(
        derechoReconstruido.alcanceTerritorial?.unidadSuperficie,
        'ha',
      );
      expect(
        derechoReconstruido.alcanceTerritorial?.referenciaGeografica,
        'Parcela rural de prueba',
      );

      expect(derechoReconstruido.tipoInstrumento, 'titulo');
      expect(
        derechoReconstruido.identificadorInstrumento,
        'TIT-001',
      );
      expect(derechoReconstruido.emisor, 'Registro de prueba');
      expect(derechoReconstruido.estado, 'vigente');

      expect(
        derechoReconstruido.condiciones,
        [
          'Sin restricciones declaradas',
          'Sujeto a verificacion documental',
        ],
      ...
      expect(
        derechoReconstruido.fechaInicio,
        DateTime(2020, 1, 1),
      );
      expect(
        derechoReconstruido.fechaCreacion,
        fecha,
      );
      expect(
        derechoReconstruido.ultimaActualizacion,
        fecha,
      );
    },
  );
